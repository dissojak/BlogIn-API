...
route.post('/signUp',(req,res,next)=>{
    ...
    bcrypt.hash(req.body.password,10).then(hash=>{
        ...
        console.log(error)
        if (error && (error.code === 11000 || (error.errors && error.errors.email && error.errors.email.kind === 'unique'))) {
...
//    .then(user=>{ 
    ...
    route.post("/followUser",checkauth,(req,res)=>{
        User.updateOne({_id:req.body.id},{$addToSet:{followers : req.userData.userId}}).then(result=>{
                User.updateOne({_id:req.userData.userId},{$addToSet:{followings : req.body.id}}).then(result=>{
